  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof AxiosError && error.response) {
        ...
        if (status === 409 && message === "This email’s already taken. Please login.") {
          toast.custom((t) => (
            <div
              className={`${
          ...
          switch (message) {
            case "We need a valid email address.":
              errorMessage = "That email doesn’t look right. Please check it.";
              break;
            case "Your phone number needs to be 4-15 digits.":
              errorMessage = "Your phone number needs to be 4-15 digits.";
              break;
            case "Your password can’t be empty or have spaces.":
              errorMessage = "Your password can’t be empty or have spaces.";
              break;
            default:
