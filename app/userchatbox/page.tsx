  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Fetch admins and set default conversation with the first admin
        const adminsResponse = await axios.get<User[]>(`${BACKEND_URL}/users/admins`, {
          params: { excludeUserId: userId },
          withCredentials: true,
        });
        const admins = adminsResponse.data.map((user) => ({ ...user, id: String(user.id) }));
        if (admins.length > 0) {
          await selectOrCreateConversation(admins[0].id);
          setShowChatMessages(true);
        }
      } catch (error) {
        console.error("Error fetching user:", error);
