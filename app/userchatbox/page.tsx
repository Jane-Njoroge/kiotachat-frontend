...
//   const selectOrCreateConversation = async (userIdToChat: string) => {
//     if (!userId) return;
//     try {
//       const existingConversation = conversations.find(
//         (conv) =>
//           (conv.participant1.id === userId && conv.participant2.id === userIdToChat) ||
//           (conv.participant1.id === userIdToChat && conv.participant2.id === userId)
//       );
...
//         { participant1Id: userId, participant2Id: userIdToChat },
...
//   // Extract unique contacted users (excluding self)
//   const contactedUsers = Array.from(
//     new Set(
//       conversations
//         .flatMap((conv) => [
//           conv.participant1.id !== userId ? conv.participant1 : conv.participant2,
//         ])
...
//               } text-center`}>No users found</div>
//             )}
...
//               </div>
//             ) : (
//               <p className="text-gray-500 dark:text-gray-400">No previous contacts found</p>
//             )}
...
const Chatbox: React.FC = () => {
  ...
  useEffect(() => {
    if (!forwardSearchQuery.trim() || !userId) {
    ...
    const search = async () => {
      try {
        const response = await axios.get<User[]>(`${BACKEND_URL}/search/users`, {
          params: { query: forwardSearchQuery, excludeUserId: userId },
          withCredentials: true,
          timeout: 10000,
        });
        setForwardSearchResults(response.data.map((user) => ({ ...user, id: String(user.id) })));
      } catch (error) {
