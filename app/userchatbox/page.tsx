...
const Chatbox: React.FC = () => {
  ...
  const menuRef = useRef<HTMLDivElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const messageQueueRef = useRef<Message[]>([]); // New: Message queue
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    const processMessageQueue = () => {
      ...
      setSelectedConversation((prev) => {
        ...
        return {
          ...prev,
          messages: [...updatedMessages, ...newMessages].sort(
      ...
      setConversations((prev) => {
        const updated = prev.map((conv) => {
          ...
          return {
            ...
            unread: selectedConversation?.id === conv.id ? 0 : conv.unread,
            updatedAt: new Date().toISOString(),
          };
        });
        return updated.sort(
    ...
    socket.on("private message", (message: Message) => {
      ...
      if (normalizedMessage.conversationId === selectedConversation?.id) {
        messageQueueRef.current.push(normalizedMessage);
        processMessageQueue();
      } else {
        setConversations((prev) => {
          const exists = prev.find((conv) => conv.id === normalizedMessage.conversationId);
        ...
        prev.map((conv) =>
          conv.id === normalizedMessage.conversationId
            ? {
                ...conv,
                messages: conv.messages
                  .map((msg) => (msg.id === normalizedMessage.id ? normalizedMessage : msg))
                  .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()),
              }
            : conv
        ).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
        ...
        prev.map((conv) =>
          conv.id === normalizedDeletedMessage.conversationId
            ? {
                ...conv,
                messages: conv.messages
                  .filter((msg) => msg.id !== normalizedDeletedMessage.id)
                  .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()),
              }
            : conv
        ).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
    ...
    socket.on("conversation updated", (updatedConversation: Conversation) => {
      ...
      if (selectedConversation?.id === normalizedConversation.id) {
        setSelectedConversation({
          ...
          messages: normalizedConversation.messages.sort(
            (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
          ),
        });
        scrollToBottom();
