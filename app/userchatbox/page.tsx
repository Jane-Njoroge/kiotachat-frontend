  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Fetch admins and auto-select the first one on login
        const adminResponse = await axios.get<User[]>(`${BACKEND_URL}/search/users`, {
          params: { query: "", excludeUserId: userId, role: "ADMIN" },
          withCredentials: true,
          timeout: 10000,
        });
        const admins = adminResponse.data.map((user) => ({ ...user, id: String(user.id) }));
        if (admins.length > 0 && !selectedConversation) {
          selectOrCreateConversation(admins[0].id);
        }
      } catch (error) {
        console.error("Error fetching user:", error);
        toast.error("Authentication failed. Please try again.", { duration: 3000 });
          ...
          >
            <div className="flex items-center gap-2">
              ...
              >
                <FontAwesomeIcon
                  icon={faArrowLeft}
                  className={`w-5 h-5 ${isDarkMode ? "text-gray-100" : "text-black"}`}
                />
              </button>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#005555] flex items-center justify-center text-white font-semibold">
